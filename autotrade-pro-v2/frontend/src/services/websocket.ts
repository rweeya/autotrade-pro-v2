...
function App() {
  ...
  // Исправленный useEffect для закрытия позиций (без ошибок)
  useEffect(() => {
                ...
                {positions.length === 0 ? (
                  ...
                  <div className="max-h-[300px] overflow-y-auto space-y-2">
                    {positions.map((pos, idx) => {
                      const profitColor = pos.profit && pos.profit > 0 ? 'text-green-400' : pos.profit && pos.profit < 0 ? 'text-red-400' : 'text-white'
                      return (
                        <div key={idx} className="bg-gradient-to-r from-red-900/20 to-black rounded-lg p-3 border-l-4 border-yellow-500 hover:bg-red-900/30 transition">
                          <div className="flex justify-between items-center">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-gray-500">Размер: {pos.qty}</div>
                            </div>
                          </div>
                          <div className="flex justify-between mt-2 text-xs">
                            <span className="text-gray-400">Открыто: {new Date(pos.openTime).toLocaleTimeString()}</span>
                            <span className={profitColor}>{pos.profit ? `${pos.profit > 0 ? '+' : ''}$${pos.profit.toFixed(2)}` : '—'}</span>
                          </div>
                        </div>
                      )
                    })}
