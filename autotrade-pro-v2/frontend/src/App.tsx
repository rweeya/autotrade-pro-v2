...
const App: React.FC = () => {
  ...
  const openBybit = (symbol: string) => {
    ...
    notification.className = 'fixed bottom-4 right-4 bg-gradient-to-r from-green-600 to-green-700 text-white px-5 py-3 rounded-xl shadow-2xl z-50 flex items-center gap-3 animate-bounce';
    notification.innerHTML = `
      <div class="text-2xl">✅</div>
      <div>
        <div class="font-bold">${cleanSymbol} скопирован!</div>
        <div class="text-xs opacity-90">Вставь в поиск Bybit (Ctrl+V)</div>
      </div>
    `;
    ...
    // Открываем Bybit
    window.open('https://www.bybit.com/ru-RU/dashboard', '_blank');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-red-900/30 to-black">
      ...
      <div className="relative z-10 container mx-auto px-6 py-6">
              ...
              {openTrades.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-black/40 text-gray-400">
                    <tr><th className="text-left p-3">МОНЕТА</th><th className="text-left p-3">ТИП</th><th className="text-right p-3">ЦЕНА ВХ.</th><th className="text-right p-3">КОЛ-ВО</th><th className="text-right p-3">TP</th><th className="text-right p-3">SL</th><th className="text-right p-3">ТЕКУЩИЙ P&L</th></tr>
                  </thead>
                  <tbody>
                    {openTrades.map(trade => {
                      ...
                      return <tr key={trade.id} className="border-b border-gray-800"><td className="p-3 font-bold">{trade.symbol}</td><td className={`p-3 ${trade.side === 'buy' ? 'text-green-400' : 'text-red-400'}`}>{trade.side === 'buy' ? 'BUY' : 'SELL'}</td><td className="p-3 text-right">${formatPrice(trade.entryPrice)}</td><td className="p-3 text-right">{trade.quantity.toFixed(4)}</td><td className="p-3 text-right text-green-400">${formatPrice(trade.tpPrice)}</td><td className="p-3 text-right text-red-400">${formatPrice(trade.slPrice)}</td><td className={`p-3 text-right font-bold ${currentPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>{currentPnL >= 0 ? '+' : ''}{formatNumber(currentPnL)} ({currentPnLPercent.toFixed(2)}%)</td></tr>;
                    })}
                  </tbody>
                </table>
              )}
        ...
        {activeTab === 'history' && (
          <div className="bg-black/40 backdrop-blur rounded-xl border border-red-500/20 overflow-hidden">
            <div className="px-5 py-3 bg-red-950/30 border-b border-red-500/30 flex justify-between">
              <h3 className="font-bold text-red-400">📜 ИСТОРИЯ СДЕЛОК ({totalTrades})</h3>
              <button onClick={clearHistory} className="text-xs text-red-400">Очистить</button>
            </div>
            <div className="overflow-x-auto max-h-[400px] overflow-y-auto">
              {totalTrades === 0 ? <div className="text-center text-gray-500 py-12">Нет закрытых сделок</div> : (
                <table className="w-full text-sm">
                  <thead className="bg-black/40 text-gray-400 sticky top-0"><tr><th className="text-left p-3">МОНЕТА</th><th className="text-left p-3">ТИП</th><th className="text-right p-3">ЦЕНА ВХ.</th><th className="text-right p-3">ЦЕНА ВЫХ.</th><th className="text-right p-3">PnL</th><th className="text-right p-3">%</th><th className="text-right p-3">ВРЕМЯ</th></tr></thead>
                  <tbody>{closedTrades.slice().reverse().map(trade => <tr key={trade.id} className="border-b border-gray-800"><td className="p-3 font-bold">{trade.symbol}</td><td className={`p-3 ${trade.side === 'buy' ? 'text-green-400' : 'text-red-400'}`}>{trade.side === 'buy' ? 'BUY' : 'SELL'}</td><td className="p-3 text-right">${formatPrice(trade.entryPrice)}</td><td className="p-3 text-right">${formatPrice(trade.exitPrice || 0)}</td><td className={`p-3 text-right font-bold ${(trade.profit || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>{(trade.profit || 0) >= 0 ? '+' : ''}{formatNumber(trade.profit || 0)}</td><td className={`p-3 text-right ${(trade.profitPercent || 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>{(trade.profitPercent || 0) >= 0 ? '+' : ''}{(trade.profitPercent || 0).toFixed(2)}%</td><td className="p-3 text-right text-gray-500 text-xs">{formatTime(trade.entryTime)}</td></tr>)}</tbody>
                </table>
              )}
            </div>
          </div>
        ...
        {activeTab === 'settings' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-black/40 rounded-xl p-6 border border-red-500/20"><h3 className="text-lg font-bold text-red-400 mb-4">🤖 АВТОТОРГОВЛЯ</h3><button onClick={() => setAutoTrade(!autoTrade)} className={`w-full py-3 rounded-lg font-bold ${autoTrade ? 'bg-red-600' : 'bg-green-600'}`}>{autoTrade ? '🔴 ОСТАНОВИТЬ' : '🟢 ЗАПУСТИТЬ'}</button>{autoTrade && <div className="mt-4 p-3 bg-green-500/20 rounded-lg"><p className="text-green-400 font-bold text-sm">✅ АВТОТОРГОВЛЯ АКТИВНА</p></div>}</div>
            <div className="bg-black/40 rounded-xl p-6 border border-red-500/20"><h3 className="text-lg font-bold text-red-400 mb-4">💰 РИСК</h3><label className="text-sm text-gray-400">Риск: {riskPercent}%</label><input type="range" min="1" max="10" step="0.5" value={riskPercent} onChange={(e) => setRiskPercent(parseFloat(e.target.value))} className="w-full accent-red-500 mt-2" /><div className="mt-4 p-3 bg-red-950/30 rounded-lg text-sm"><div className="flex justify-between"><span className="text-gray-400">Баланс:</span><span>${formatNumber(balance)}</span></div><div className="flex justify-between"><span className="text-gray-400">Сумма на сделку:</span><span className="text-yellow-400">${formatNumber(balance * riskPercent / 100)}</span></div></div></div>
            <div className="bg-black/40 rounded-xl p-6 border border-red-500/20"><h3 className="text-lg font-bold text-red-400 mb-4">⚙️ УПРАВЛЕНИЕ</h3><button onClick={resetBalance} className="w-full py-2 bg-yellow-600/50 rounded-lg mb-3">🔄 Сбросить баланс</button><button onClick={clearHistory} className="w-full py-2 bg-red-600/50 rounded-lg">🗑️ Очистить историю</button></div>
            <div className="bg-black/40 rounded-xl p-6 border border-red-500/20"><h3 className="text-lg font-bold text-red-400 mb-4">ℹ️ СТАТУС</h3><div className="space-y-2 text-sm"><div className="flex justify-between"><span className="text-gray-400">WebSocket:</span><span>{wsConnectedCount}/{SYMBOLS.length}</span></div><div className="flex justify-between"><span className="text-gray-400">Открыто позиций:</span><span>{openTrades.length}</span></div><div className="flex justify-between"><span className="text-gray-400">Винрейт:</span><span className="text-green-400">{winRate.toFixed(1)}%</span></div></div></div>
          </div>
        )}
        ...
        {activeTab === 'signals' && (
          <div className="space-y-3">
            {signals.length === 0 ? (
              <div className="bg-black/40 rounded-xl p-12 text-center"><div className="text-6xl mb-4">⏳</div><div className="text-gray-400">Нет сигналов. Ожидаем RSI {'<'}30 или RSI {'>'}70...</div></div>
            ) : (
              signals.map((signal, idx) => {
                const stars = '★'.repeat(signal.strength) + '☆'.repeat(3 - signal.strength);
                return (
                  <div key={idx} onClick={() => openBybit(signal.symbol)} className="bg-gradient-to-r from-black/60 to-red-900/20 rounded-xl p-4 border border-red-500/30 hover:border-red-500/50 cursor-pointer transition">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3"><span className="text-2xl">{signal.action === 'buy' ? '🟢' : '🔴'}</span><div><div className="font-bold text-lg">{signal.symbol}</div><div className="text-xs text-gray-500">{formatTime(signal.timestamp)}</div></div></div>
                      <div className={`px-4 py-1.5 rounded-lg text-sm font-bold ${signal.action === 'buy' ? 'bg-green-600' : 'bg-red-600'}`}>{signal.action === 'buy' ? 'BUY' : 'SELL'} @ ${formatPrice(signal.price)}</div>
                      <div className="text-yellow-400 text-sm">{stars}</div>
                    </div>
                    <div className="grid grid-cols-4 gap-3 mt-3 text-xs">
                      <div className="bg-black/50 rounded-lg p-2 text-center"><div className="text-gray-500">RSI</div><div className="font-bold">{signal.rsi.toFixed(1)}</div></div>
                      <div className="bg-black/50 rounded-lg p-2 text-center"><div className="text-gray-500">MACD</div><div className="font-mono">{signal.macd > 0 ? '+' : ''}{signal.macd.toFixed(4)}</div></div>
                      <div className="bg-black/50 rounded-lg p-2 text-center"><div className="text-gray-500">EMA</div><div className="text-xs">{signal.ema20.toFixed(0)}/{signal.ema50.toFixed(0)}</div></div>
                      <div className="bg-black/50 rounded-lg p-2 text-center"><div className="text-gray-500">Сила</div><div className="text-yellow-400">{stars}</div></div>
                    </div>
                    <div className="mt-2 text-xs text-red-400 flex gap-2 flex-wrap">{signal.reasons.map((r,i) => <span key={i} className="bg-red-950/30 px-2 py-0.5 rounded">🎯 {r}</span>)}</div>
                  </div>
                );
              })
            )}
